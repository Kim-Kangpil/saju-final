...
class GPTInterpretationGenerator:
    ...
    def generate_element_interpretation(self, element_counts, tone='empathy', theories='', analysis=None):
        ...
        # 톤별 시스템 프롬프트
        tone_prompts = {
            'empathy': "당신은 사주명리학 30년 경력의 전문 상담가입니다. 운명론적 결정론보다는 사람의 잠재력과 가능성에 집중하며, 따뜻하고 공감적인 어조로 격려와 지지의 메시지를 전달합니다.",
...
"""
        ...
        user_prompt = f"""다음 사주를 분석해서 종합 리포트를 작성해줘.

{analysis_info}{theory_section}

[절대 지켜야 할 규칙]
1. 마크다운 기호(##, **, *, -) 절대 사용 금지. 소제목은 이모지+한글로만 표현.
2. 사주 전문 용어(일간, 십성, 오행, 편재, 정관 등) 직접 언급 금지. 일상 언어로만 설명.
3. 타이밍 확정 금지. "~할 것이다" 대신 "~하는 흐름이 있어요" 형태로.
4. 추측하거나 지어내지 말 것. 위 계산 데이터에 있는 것만 설명.
5. 존댓말로 작성.

[리포트 구성 — 이 순서대로 빠짐없이 작성]

🔮 한 줄 핵심 진단
이 사람의 인생 구조를 한 문장으로. 읽자마자 "맞아" 하게 만들 것.

🧠 타고난 성향과 사고방식
어떻게 생각하고 결정하는지. 감정 처리 방식. 남들이 보는 이 사람 vs 실제 이 사람.

💪 이 사람의 진짜 무기
타고난 강점. 어떤 방식으로 힘을 발휘하는지. 유리한 상황.

🔁 반복되는 문제 패턴
왜 같은 문제가 반복되는지. 무의식적 습관. 조심해야 할 패턴.

💰 돈 흐름 구조
돈을 버는 방식. 돈이 안 모이는 이유. 재물운 패턴.

🧭 일과 진로 방향
잘 맞는 일의 종류. 조직 vs 독립. 커리어에서 유리한 방식.

❤️ 관계와 연애 성향
기본 관계 패턴. 연애에서 반복되는 구조. 잘 맞는 사람 유형.

⏰ 지금 이 시기
지금 왜 이런 상황인지. 현재 흐름의 성격. 무엇에 집중해야 하는지.
...
""" + (
    ...
    def generate_core_values(
        ...
        tone_prompts = {
            'empathy': "당신은 사주명리학 30년 경력의 전문 상담가입니다. 결정론적으로 단정 짓지 말고, 가능성과 선택지를 열어두면서 따뜻하고 공감적인 어조로 사용자의 마음을 존중하세요.",
...
"""
    ...
    def generate_section1(self, analysis, tone='empathy'):
        ...
        # 3. 톤별 시스템 메시지
        system_prompts = {
            'empathy': "당신은 사주명리학 30년 경력의 전문 상담가입니다. 부드럽고 공감적인 어조로 긍정적 메시지를 전달하되, 현실적인 조언도 함께 제공합니다.",
