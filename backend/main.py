...
• 아래 섹션 구조는 내용 흐름 참고용이며, 출력물에 레이블로 표기 금지:
  행동/상황으로 시작하는 첫 문장 → 왜 그런지 2~3문장 → 실제 삶에서 어떻게 나타나는지 2~3문장 → 지금 이 시기와 연결 1~2문장
...
2. 톤 규칙 (공감적이고 따뜻하게):
   부정적 사실을 말할 때는 먼저 공감/상황 묘사로 시작 ("~하죠?", "~한 느낌 들죠?")하고, 계산된 데이터 기반 사실을 전달한 뒤, "하지만" 또는 "그 대신"으로 실제 가능한 출구를 제시하는 흐름으로 써줘.
   각 섹션은 독자가 "맞아, 나 이래" 느끼는 공감 문장으로 시작해서, 왜 그런지 사실을 풀고, 실제 삶에서 어떻게 나타나는지 구체적 예시를 거쳐, 출구/전략(데이터에서 실제로 가능한 것만)으로 마무리하는 흐름으로 작성.
   데이터 근거가 있을 때만 희망 신호 가능 — 근거 없는 희망 절대 금지.
