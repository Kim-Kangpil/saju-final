...
/** 일간 기질 설명 테이블 — GPT에게 일간의 성격·이미지를 알려줌 */
const ILGAN_DESC: Record<string, string> = {
  甲: "갑목(양목) — 큰 나무·대들보. 직진적이고 진취적, 리더 기질",
  乙: "을목(음목) — 덩굴·화초. 유연하고 적응력 강함, 친화력 높음",
  丙: "병화(양화) — 태양. 화려하고 열정적, 밝은 에너지, 사교성",
  丁: "정화(음화) — 촛불·난로. 섬세하고 따뜻한 내면의 빛, 집중력",
  戊: "무토(양토) — 산·대지. 묵직하고 안정적, 포용력, 신뢰감",
  己: "기토(음토) — 논밭·습지. 세심하고 분석적, 현실적, 관리 능력",
  庚: "경금(양금) — 바위·쇠. 결단력 있고 강직함, 의리, 완고함",
  辛: "신금(음금) — 보석·칼날. 예리하고 완벽주의적, 심미안, 자존심",
  壬: "임수(양수) — 강·바다. 활동적이고 지략 넓음, 포용력, 기획력",
  癸: "계수(음수) — 이슬·안개비. 섬세하고 직관력·흡수력 강함, 감수성",
};

/** 월지/지지 계절·이미지 설명 테이블 */
const BRANCH_DESC: Record<string, string> = {
  子: "자수 — 겨울 한가운데, 응집된 생명력, 지혜롭고 은밀함",
  丑: "축토 — 겨울 끝 얼어붙은 땅, 인내·축적의 기운",
  寅: "인목 — 초봄 깨어남, 도전·활동성·진취",
  卯: "묘목 — 봄 한가운데 꽃피움, 부드러움·창의·감성",
  辰: "진토 — 늦봄 봄비의 달, 총명·비밀스러움·변화",
  巳: "사화 — 초여름 뜨거워지는 태양, 지략·분석·활동성",
  午: "오화 — 여름 한가운데 절정의 빛, 직관·열정·명예욕",
  未: "미토 — 늦여름 건조한 땅, 섬세함·예술성·안정 추구",
  申: "신금 — 초가을 수렴·결실, 다재다능함·행동력",
  酉: "유금 — 가을 한가운데 완성·수확, 예리함·정제된 미감",
  戌: "술토 — 늦가을 건조·쇠락, 지략·경계심·충성심",
  亥: "해수 — 초겨울 씨앗의 잠, 직관·영적 감수성·탐구심",
};
...
[좋은 답변 예시 — 이 스타일로만 답변할 것]

예시 1)
Q: 내 재물운 어때?
A: 돈이 한 번에 크게 들어오기보다 조금씩 꾸준히 쌓이는 구조예요.
근데 문제는 나가는 속도도 비슷하다는 거예요.

특히 사람 때문에 돈이 나가는 경우가 많아요.
밥 사주고, 도와주고, 빌려주고— 그런 패턴이요.

지금 이 시기는 혼자 벌려고 하기보다
누군가와 함께하는 구조를 만들 때 돈이 더 잘 모여요.
1~2년 안에 수입 구조 자체가 바뀔 가능성이 있어요.

예시 2)
Q: 나 연애운은?
A: 인연이 없는 게 아니에요.
다가오는 사람은 있는데 본인이 잘 안 열어요.

처음엔 좋아 보여도 시간이 지나면서
"이 사람이 맞나?" 하는 의심이 올라오는 타입이에요.
그래서 관계가 깊어지기 직전에 멈추는 경우가 많아요.

지금 시기는 새로운 인연보다
이미 알던 사람 중에서 가능성이 생기는 흐름이에요.
억지로 만들려고 하면 오히려 안 되고
자연스럽게 흘러가게 두는 게 맞아요.

예시 3)
Q: 요즘 왜 이렇게 힘들지?
A: 지금 뭔가를 끝내야 하는 시기인데
아직 새로운 것도 안 잡혀있는 상태예요.
...
이게 무능한 게 아니에요.
이 시기가 원래 버티는 구간이에요.
억지로 뭔가 만들려고 할수록 더 힘들어져요.

지금은 크게 결정하기보다
작은 것들을 하나씩 정리하는 게 맞는 타이밍이에요.
...
[해석 품질 가이드 — 반드시 준수]
1. 답변 시작은 반드시 이 사람의 사주 특성으로 ("일반적으로" 금지)
2. 질문과 관련된 규칙 기반 해석 포인트를 반드시 첫 문단에 녹여라
3. 합충이 있으면 반드시 해석에 반영
4. 현재 대운이 있으면 지금 시기와 연결
5. 최소 500자 이상, 가능하면 700자 이상 깊이 있게 답변
6. 사주 데이터에 없는 내용은 절대 추측하지 말 것
7. 문장은 짧고 단정하게. "~예요", "~있어요" 형태로
8. 이 사람만의 구체적인 패턴을 말해라. "보통 이런 타입은" 금지
9. 읽다가 "맞아, 나 얘기네" 반응이 나오게 써라
10. 규칙 기반 해석의 language_points 문장을 그대로 활용해라
`;
...
- "이어서 보면 좋은 질문"에는 정확히 2개만 제시한다. 번호 없이 줄바꿈으로만 구분한다.
- 이 2개 질문은 반드시 (가) 방금 답한 주제·키워드와 정확히 같은 주제의 사주·명리 질문일 것. (나) 아래는 절대 금지: 이번 주 행동 바꾸기, 실수 반복·습관, 동기부여, 멘탈, 자기계발 코칭, 사주와 무관한 문장.
- 주제 일치 필수: 재물 질문 → 재물·수입 관련 사주 질문만. 연애 질문 → 연애·결혼 관련만. 직업 질문 → 직업·커리어 관련만. 건강 질문 → 건강·체력 관련만. 다른 주제(귀인·신살 등)로 갑자기 전환하는 것 절대 금지.
- 좋은 예(재물 질문 후): "재물운이 가장 좋아지는 대운은 언제부터야?", "재성이 합에 걸리면 돈이 어떻게 바뀌어?"
- 좋은 예(연애 질문 후): "내 사주에서 배우자 인연이 들어오는 시기를 볼 수 있어?", "관성이 충을 받으면 연애에 어떤 영향을 줘?"
- 나쁜 예(금지): 재물을 물었는데 "천을귀인이 있는지 어떻게 봐?" — 주제가 달라서 금지.
- 마지막은 반드시 질문 2개로 끝낸다.`;
