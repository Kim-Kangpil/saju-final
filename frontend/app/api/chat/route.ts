...
const RESPONSE_FORMAT_RULE = `[응답 형식 — 반드시 준수]
- 질문에 직접 답하는 것만. 서론·도입부 금지.
- 분량: 3~5문장. 질문이 단순하면 2문장도 충분.
...
export async function POST(req: Request) {
  ...
  const system = [
    ...
    interpretationBlock || (hasSaju ? "\n[주의: 규칙 기반 해석 데이터 없음. 사주 컨텍스트 블록 데이터만으로 답변할 것. 추측 금지.]" : ""),
    MONTH_BRANCH_RULE,
    responseFormatBlock,
    theoryBlock,
    "\n\n[이번 질문에 필요한 사주 지식]",
