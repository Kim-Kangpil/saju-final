  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <InicisPayButton orderType="love" price={price} label="연애운 리포트 구매" sajuId={sajuId} onError={setPayErr} />
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        const payload = await loadReportInputBySajuId(sajuId);
        if (!payload) throw new Error("사주 데이터를 불러오지 못했어요.");
        const res = await fetch(`${API_BASE}/saju/report/love-realistic`, {
          method: "POST", credentials: "include",
          headers: { "Content-Type": "application/json", ...getAuthHeaders() },
          body: JSON.stringify(payload),
        });
  ...
  return (
    <main style={{ minHeight: "100vh", background: "#F5F1EA", fontFamily: "var(--font-sans), 'Gmarket Sans', sans-serif", color: "#2C2417" }}>
      ...
      {showAddonModal && sajuId && (
        <LoveAddonModal price={addonPrice} sajuId={sajuId} onDismiss={() => setShowAddonModal(false)} />
      )}
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              <button
                ...
                onClick={async () => {
                  ...
                  if (navigator.share) {
                    try { await navigator.share({ title: '연애운 분석 리포트', url }); } catch {}
                  } else {
