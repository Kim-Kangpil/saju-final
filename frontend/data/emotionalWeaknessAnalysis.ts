...
export function getEmotionalWeaknessParagraph(
  ...
  bodyParts.push(describeEmotionAxis(inLvl, biLvl));

  // 2) 스트레스가 쌓이는 이유 — 관성
  bodyParts.push(describeGwanSeong(gwanLvl));

  // 3) 감정이 폭발하는 방식 — 식상
  bodyParts.push(describeSikSang(sikLvl));

  // 4) 관계에서 소진되는 이유 — 재성
  bodyParts.push(describeJaeSeong(jaeLvl));
