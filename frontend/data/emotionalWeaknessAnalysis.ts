...
function describeGwanSeong(level: "none" | "weak" | "mid" | "strong"): string {
  if (level === "strong" || level === "mid") {
    return "책임감과 완벽주의가 강해 스스로를 끊임없이 압박하는 구조예요. 평가받는 상황, 실수, 기대에 못 미치는 장면에서 극도로 예민해지기 쉽고, 잘해야 한다는 강박이 일상적으로 작동합니다. 👉 감정 약점: 완벽주의 압박, 평가 공포, 실수 자책";
  }
  if (level === "weak") {
    return "사회적 책임과 규칙에 적당히 반응하면서도, 요구가 몰릴 때는 부담을 크게 느끼는 편이에요. 해야 할 일을 알고는 있지만, 가끔은 '혹시 내가 부족한 건 아닐까' 하는 긴장이 함께 따라붙습니다. 👉 감정 약점: 책임 스트레스 (보통 수준)";
  }
  return "외부의 평가나 규칙에 크게 구애받지 않는 편이라 자유로운 대신, 자기 규율이 약해지거나 책임을 회피하는 방향으로 흐르기 쉽습니다. 해야 할 일을 미루다가 막판에 몰려 스트레스를 받는 패턴이 생길 수 있어요. 👉 감정 약점: 책임 회피, 규칙 거부감";
}

function describeSikSang(level: "none" | "weak" | "mid" | "strong"): string {
  if (level === "strong" || level === "mid") {
    return "감정을 바로 말이나 행동으로 표출하는 편이에요. 표현 자체는 시원하지만 감정이 격해지면 말이 강해지고, 상대에게 상처를 주는 표현이 튀어나와 놓고 나서 후회하는 패턴이 반복되기 쉽습니다. 👉 감정 약점: 말 실수, 감정 과잉 표출, 후회 반복";
  }
  if (level === "weak") {
    return "감정 표현이 상황에 따라 크게 달라지는 타입이에요. 편안한 관계에서는 잘 풀어내지만, 낯선 상황이나 권위 앞에서는 억누르면서 스스로 답답함을 안고 가기 쉽습니다. 👉 감정 약점: 상황 의존적 표현";
  }
  return "감정을 밖으로 꺼내는 통로가 막혀 있는 쪽에 가까워요. 하고 싶은 말을 못 하고 쌓아두다가 두통·소화불량 같은 몸 증상으로 나타나거나, 어느 순간 한 번에 무너지는 방식으로 터질 수 있습니다. 👉 감정 약점: 감정 억압, 신체화 증상, 말 못하고 쌓음";
}

function describeJaeSeong(level: "none" | "weak" | "mid" | "strong"): string {
  if (level === "strong" || level === "mid") {
    return "사람을 의식하고 관계를 관리하는 데 많은 에너지를 쓰는 편이에요. 눈치가 빠르고 상대 반응에 민감하게 반응하다 보니 관계 피로가 쌓이고, 잘 보이려는 노력이 오히려 스트레스가 되기도 합니다. 👉 감정 약점: 눈치 과부하, 관계 피로, 거절 못함";
  }
  if (level === "weak") {
    return "관계에서 적당한 균형을 유지하려는 타입이에요. 지나치게 의식하지도, 완전히 무시하지도 않는 대신, 가끔은 '어디까지 맞춰야 하지?' 하는 애매함 속에서 에너지를 쓰게 됩니다. 👉 감정 약점: 관계 스트레스 (보통 수준)";
  }
  return "인간관계에서 타인의 감정을 읽는 것에 다소 둔감한 편이에요. 상처를 주고도 잘 모르거나, 반대로 본인이 상처를 받아도 명확히 인식하지 못한 채 쌓아 두는 경우가 생길 수 있습니다. 👉 감정 약점: 공감 부족, 관계 단절, 감정 둔감";
}
...
export function getEmotionalWeaknessParagraph(
  ...
  // 도입부
  if (tone === "fun") {
    parts.push("감정 약점을 본다는 건 '어디가 약하니까 망한다'가 아니라, 어디서 더 쉽게 흔들리는지 알고 관리 포인트를 찾는 거야. 네 사주는 인성·비겁·관성·식상·재성 분포를 보면 이런 식으로 감정 에너지가 배치되어 있어.");
  } else if (tone === "reality") {
    parts.push("감정 약점은 결함이라기보다, 어느 축에서 더 에너지를 많이 쓰고 어디서 쉽게 흔들리는지 보여 주는 지표에 가깝습니다. 이 사주는 인성·비겁·관성·식상·재성의 분포를 기준으로 감정 구조와 스트레스 포인트를 이렇게 읽을 수 있어요.");
  } else {
    parts.push("감정 약점은 '내가 잘못됐다'는 증거가 아니라, 어느 부분에서 조금 더 지지와 휴식이 필요한지 알려주는 지점이에요. 인성·비겁·관성·식상·재성의 분포를 보면, 당신의 감정 에너지는 이렇게 움직입니다.");
  }

  // 1) 감정이 흔들리는 상황 — 인성/비겁 조합
  parts.push(describeEmotionAxis(inLvl, biLvl));

  // 2) 스트레스가 쌓이는 이유 — 관성
  parts.push(describeGwanSeong(gwanLvl));

  // 3) 감정이 폭발하는 방식 — 식상
  parts.push(describeSikSang(sikLvl));

  // 4) 관계에서 소진되는 이유 — 재성
  parts.push(describeJaeSeong(jaeLvl));

  // 편/정 비율에 따른 감정 색깔
  const bias = describeBias(detail);
  if (bias.length > 0) {
    parts.push(bias.join(" "));
  }

  // 마무리: 시간축
  if (tone === "fun") {
    parts.push("이 감정 패턴들은 한 번에 고치려고 하기보다, '아 내가 이런 패턴이 있구나'를 알고 한두 군데만 의식적으로 다루기 시작하면 서서히 힘이 빠지는 구조야. 시간이 지날수록, 지금의 약점이 오히려 남들보다 감정에 섬세한 무기가 되는 쪽으로 바뀌어 가게 돼.");
  } else if (tone === "reality") {
    parts.push("이 축들은 단기간에 바뀌기보다는, 패턴을 인식하고 작은 선택을 반복하면서 천천히 재구조화됩니다. 시간을 두고 자신만의 관리법을 만들어 갈수록, 같은 자극에서도 덜 흔들리고 더 전략적으로 감정을 활용하는 능력으로 전환될 수 있어요.");
  } else {
    parts.push("이 감정 축들은 지금 이 모습 그대로 고정된 운명이 아니라, 시간이 흐르면서 조금씩 다루는 법을 익혀 가는 연습장에 가깝습니다. 오늘 이후로는 같은 상황을 만날 때마다 '아, 여기서 내 패턴이 올라오는구나'라고 알아차리는 것만으로도, 약점이 서서히 무기로 바뀌는 흐름을 만들 수 있어요.");
  }
