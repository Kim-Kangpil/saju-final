...
function summarizeByKey(
  ...
  switch (key) {
    case "cheonul":
      return `천을귀인은 위기 순간마다 사람이나 사건의 형태로 '기묘한 도움'이 들어오는 구조예요${posText}. 관재·구설·사고처럼 막다른 길처럼 보이는 국면에서도, 끝까지 가 보면 반전의 구멍이 열리는 경우가 많아서 살면서 몇 번이고 '운이 좋았다'는 말을 듣게 되는 타입입니다.`;
    case "cheondeok":
      return `천덕귀인은 하늘이 덕을 얹어 둔 구조라, 큰 위험을 겪어도 이상하리만큼 비켜 가거나 완충 장치가 들어오는 경우가 많아요${posText}. 선한 태도와 덕스러운 품성이 자연스럽게 쌓여서, 시간이 지날수록 신뢰·평판이라는 형태의 보호막이 두꺼워지는 길성입니다.`;
    case "woldeok":
      return `월덕귀인은 조용히 쌓인 음덕과 내조의 기운이에요${posText}. 겉으로 요란하지 않아도 꾸준히 믿음을 주는 사람으로 남게 되고, 관계 속에서 받은 작은 도움과 배려가 위기 때 큰 힘으로 돌아오는 구조라, 나이가 들수록 '내 편'이 또렷해지는 흐름을 만듭니다.`;
    case "munchang":
      return `문창귀인은 언어·문장·표현력 쪽으로 빛이 나는 길성이에요${posText}. 글쓰기·강의·콘텐츠처럼 말과 글을 다루는 영역에서 감이 빠르고, 시험·자격 분야에서도 이해와 정리가 빨라서 '설명 잘하는 사람'으로 기억되기 쉽습니다.`;
    case "hakdang":
      return `학당귀인은 배우고 가르치는 재능에 힘이 실린 구조예요${posText}. 공부한 것을 오래 기억하고 체계적으로 정리하는 능력이 좋아서, 선생·연구·교육·코칭처럼 지식을 다루는 역할에서 자신의 무기를 만들기 좋습니다.`;
    case "cheonui":
      return `천의성은 치유와 회복의 기운이라, 남의 아픔을 민감하게 알아차리고 도와주려는 마음이 자연스럽게 올라오는 자리예요${posText}. 의료·상담·복지처럼 사람을 돌보는 영역과 연결하면, 공감 능력이 곧 직업적 강점으로 이어지기 쉬운 구조입니다.`;
    case "amrok":
      return `암록은 겉으로는 티가 잘 나지 않는 숨은 재복이에요${posText}. 표면적으로는 아슬아슬해 보여도 막판에 버텨지고, 예상 밖의 경로로 도움이 들어와서 '어떻게든 굶지 않고 버텨진다'는 식의 서사를 만드는 버팀목 역할을 합니다.`;
    case "wolgong":
      return `월공은 세속적인 욕심보다 의미와 가치를 중시하게 만드는, 맑고 가벼운 기운이에요${posText}. 물질적 결과가 다소 들쭉날쭉해 보여도 정신적으로는 빨리 정리하고 털어내는 힘이 있어서, 결국엔 나에게 남길 것과 내려놓을 것을 구분하는 감각이 또렷해집니다.`;
    case "geonrok":
      return `건록은 일간의 힘이 가장 또렷하게 서는 자리라, '내 힘으로 일어서는 독립성'을 상징해요${posText}. 직업·역할에서 꾸준히 버티는 힘이 강하고, 환경 탓보다는 스스로 해내려는 성향 덕분에 시간이 갈수록 자수성가형 서사가 잘 쌓이는 구조입니다.`;
    case "geumyeolog":
      return `금여록은 품격 있는 인연과 배우자 복을 상징하는 길성이에요${posText}. 사람을 만날수록 인연의 질이 높아지기 쉬워서, 함께하는 파트너나 배우자 쪽에서 삶의 품격이 한 단계 올라가는 경험을 만들기 좋습니다.`;
    case "bokseong":
      return `복성귀인은 일상 곳곳에서 크고 작은 복이 끊기지 않는 구조예요${posText}. 먹을 복·사람 복·생활 복이 고르게 들어와서, 큰 부자가 아니어도 '어디 가서 굶지는 않는다'는 느낌의 안전망이 항상 뒤에 깔려 있는 타입입니다.`;
    case "taegeuk":
      return `태극귀인은 인생의 굴곡 속에서도 결국 중심 자리로 돌아오는 복원력을 의미해요${posText}. 큰 흐름을 보는 감각이 좋아서, 한때 흔들려도 중요한 국면에서는 다시 균형을 찾아가는 힘이 있고, 종교·철학·역학처럼 근본을 탐구하는 분야와도 인연이 잘 닿는 구조입니다.`;
    case "cheonju":
      return `천주귀인은 먹고사는 문제와 손님 대접, 베풂의 기운이 강하게 들어오는 길성이에요${posText}. 요식·서비스·대접하는 일과 인연이 닿기 쉽고, 사람을 잘 먹이고 챙기는 태도가 곧 재복과 인복으로 돌아오기 좋은 구조입니다.`;
    case "samgi": {
