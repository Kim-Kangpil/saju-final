...
const APTITUDE_BODY: Record<string, { success: Record<AptitudeToneKey, string>; fail: Record<AptitudeToneKey, string> }> = {
  ...
  정인: {
    success: {
      empathy: "배우고 가르치는 쪽이 잘 맞아요. 지식을 쌓고 나누는 걸 좋아하고, 공부·문서·자격처럼 나를 채우는 일에서 잘 움직여요. 교육, 연구, 강사, 멘토처럼 배움을 나누는 일이 잘 어울려요. 새로운 걸 알아가거나 정리해서 전달할 때 에너지가 잘 올라오는 편이라, 강의·집필·컨설팅처럼 ‘알린 다음 나눠 주는’ 역할이 자연스럽게 잘 맞아요. 자격증이나 논문, 강의 준비처럼 단계를 밟아 가는 일도 잘 맞고, 꾸준히 공부하고 자격을 쌓는 걸 부담이라기보다 기회로 느끼는 편이고, 그만큼 전문성이 쌓이면 주변에서도 인정받기 쉬워요. 다만 혼자만 아는 데 그치지 말고, 배운 걸 정리해서 말이나 글로 꺼내 보시면 재능이 더 잘 드러나요. 이렇게 배움과 나눔을 이어가는 일이 당신에게 잘 맞는 길이에요.",
